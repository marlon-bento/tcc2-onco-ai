...
class GatedGCNLayer(pyg_nn.conv.MessagePassing):
    ...
    def __init__(self, in_dim, out_dim, dropout, residual, act= ReLU(),
        ...
        self.activation = act#register.act_dict[act]
        self.A = pyg_nn.Linear(in_dim, out_dim, bias=True) #MLP([in_dim, in_dim, out_dim])     #
        ...
        self.bn_edge_features = LayerNorm(out_dim) #nn.BatchNorm1d(out_dim)
        self.bn_node_x = LayerNorm(out_dim) #nn.BatchNorm1d(out_dim)
        self.bn_edge_e = LayerNorm(out_dim) #nn.BatchNorm1d(out_dim)
    ...
    def message(self, Dx_i, Ex_j, PE_i, PE_j, Ce):
        ...
        {}e             : [n_edges, out_dim]
        """
        e_ij = F.relu(self.bn_edge_features(Dx_i + Ex_j + Ce))  ##ADDING BN AND RELU 
