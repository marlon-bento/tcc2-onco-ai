...
def salva_modelo_treinado(model_name, dim_folder_name, tipo_lesao, n_nodes, run_id):
    """Salva o modelo com um nome único para o experimento. (Generalizado)"""
    ...
    save_dir = os.path.join(
        diretorio_base, 
        "weights", 
        f"{model_name}_{tipo_lesao}_{dim_folder_name}_{n_nodes}_RUN{run_id}.pth"
    )
...
# 4. Grid de Hiperparâmetros ajustado para HIGSI
param_grid = {
...
def run_experiment_sequential(params): 
    ...
    train_val_stratify_labels = [g.source for g in train_val_data] if hasattr(train_val_data[0], 'source') else [g.y.item() for g in train_val_data]
    ...
    run_name = f"{TIPO_LESÃO}_{TIPO_SEGMENTAÇÃO}_E{EMBEDDING_SIZE}_LR{LEARNING_RATE}_WD{WEIGHT_DECAY}_S{RANDOM_SEED}"
    best_val_accu = 0
    patience_counter = 0

    save_dir = salva_modelo_treinado("HIGSI", dim_folder_name, TIPO_LESÃO, N_NODES_A_TESTAR, run_name)
        ...
        with torch.no_grad():
            for data in test_loader:
                ...
                out = best_model(
                    ...
                    reduced_index=data.reduced_index
                )
...
def main():
    ...
    for combination in tqdm(experiment_combinations, 
        ...
        combo_name = (f"{combination['TIPO_LESÃO']}-{combination['TIPO_SEGMENTAÇÃO']} | "
                      ...
                      f"E: {combination['EMBEDDING_SIZE']}")
        ...
        N_NODES_A_TESTAR = combination['N_NODES_A_TESTAR']
        csv_path = os.path.join(diretorio_base, f"SUPERPIXEL_{N_NODES_A_TESTAR}_HIGSI.csv")
    ...
    if not results_df.empty:
        ...
        cols_to_print = [
            'TIPO_LESÃO', 'TIPO_SEGMENTAÇÃO', 'N_NODES', 'EMBEDDING_SIZE',
